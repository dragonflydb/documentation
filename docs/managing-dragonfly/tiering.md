...
- With experimental hash offloading enabled (`--tiered_experimental_hash_support`), hashes in database `0` are not offloaded while any Search index on hashes exists, including hashes that do not match the index prefix. Offloaded hashes that match a newly created index are loaded back into memory. Hashes in other databases are still offloaded.
- In cluster mode, `DFLYCLUSTER GETSLOTINFO` reports the bytes each slot holds on disk as `tiered_bytes`. The field is omitted for slots with no offloaded data.
