...
- **Dragonfly Cloud performs data store updates with zero downtime for Swarm (cluster mode) and near zero downtime (typically under 5 seconds) for single-node (emulated single-shard) data stores**.
