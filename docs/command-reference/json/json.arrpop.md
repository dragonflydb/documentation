...
`JSON.ARRPOP` returns an [array](https://valkey.io/topics/protocol/#arrays) of bulk string replies for each path, each reply is the popped JSON value, or `nil`, if the matching JSON value is not an array.
