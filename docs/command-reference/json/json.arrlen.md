...
`JSON.ARRLEN` returns an [array](https://valkey.io/topics/protocol/#arrays) of integer replies, an integer for each matching value, each is the array's length, or `nil`, if the matching value is not an array.
